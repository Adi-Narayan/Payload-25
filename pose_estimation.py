...
def images_to_video(image_folder: str, output_video: str, fps: int = 30) -> None:
    """
    Convert a sequence of images into a video.

    Args:
        image_folder (str): Path to the folder containing processed images.
        output_video (str): Path to save the output video file.
        fps (int): Frames per second for the video. Defaults to 30.
    """
    image_files = sorted(glob.glob(os.path.join(image_folder, "*.jpg")))  # Sort images by name
    if not image_files:
...
if __name__ == "__main__":
    ...
    images_to_video("output_images", output_video_path, fps=30)
# End-of-file (EOF)
