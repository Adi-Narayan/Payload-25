...
class PoseEstPostProcessing:
    ...
    def post_process(self, raw_detections: dict, height: int, width: int, class_num: int) -> dict:
        logger.debug(f"Raw detections keys: {list(raw_detections.keys())}")
        for key, value in raw_detections.items():
            logger.debug(f"Raw detection {key} shape: {value.shape}, min: {value.min()}, max: {value.max()}")
    ...
    def extract_pose_estimation_results(self, endnodes: List[np.ndarray], height: int, width: int, class_num: int) -> Dict[str, np.ndarray]:
        ...
        logger.debug(f"Extracted scores shape: {scores.shape}, min: {scores.min()}, max: {scores.max()}")
        
        kpts = [np.reshape(c, (-1, c.shape[1] * c.shape[2], 5, 3)) for c in endnodes[2:9:3]]
        ...
        logger.debug(f"Output scores shape: {output['scores'].shape}, min: {output['scores'].min()}, max: {output['scores'].max()}")
        return output

    def visualize_pose_estimation_result(self, results: dict, img: Image.Image, detection_threshold: float = 0.5, joint_threshold: float = 0.5) -> np.ndarray:
        ...
        for (detection_box, detection_score, detection_keypoints,
            ...
            detection_info = {
                'score': float(detection_score.item()),
                'width': int(xmax - xmin),
    ...
    def decoder(self, raw_boxes: np.ndarray, raw_kpts: np.ndarray, strides: List[int], image_dims: Tuple[int, int], reg_max: int) -> Tuple[np.ndarray, np.ndarray]:
        boxes = None
        decoded_kpts = None
    ...
    def non_max_suppression(self, prediction: np.ndarray, conf_thres: float = 0.1, iou_thres: float = 0.45, max_det: int = 100, n_kpts: int = 5) -> List[dict]:
...
def output_data_type2dict(hef: HEF, data_type: str) -> dict:
    return {info.name: data_type for info in hef.get_output_vstream_infos()}